...
namespace RiggVar.FR
{
    ...
    /// <summary>
    /// LocalBridge alias ProxyData
    /// 
    /// * LocalBridge ergänzt (nur) die Datenfelder
    /// und implementiert das Bridge-Interface.
    /// Die Daten sind 'lokal' vorhanden, also nicht remote.
    /// LocalBridge ist Grundlage für eine serverseitige Implementierung des
    /// Bridge-Protokolls.
    /// 
    /// * Es wird WebBridge der Zugriff auf die Daten ermöglicht,
    /// ohne dass ein zirkulärer Bezug entsteht.
    /// 
    /// WebBridge hat eine LocalBridge.
    /// ServerBridge ist eine LocalBridge;
    /// ServerBridge hat eine WebBridge;
    /// 
    /// * Eine abgeleitete Klasse (ServerBridge in Unit BridgeServer)
    /// ergänzt Konnektivität via Socket (BridgeNCP) und Web (WebBridge)
    /// 
    /// * is base class of TProxyData = class(TLocalBridge)
    /// 
    /// used in RA21
    /// 
    /// </summary>
    public class TLocalBridge : IBridgeService
    {
        private TStringList FDiffLog;
        public TStringList SLReport;
        ...
        public TLocalBridge(int Capacity)
        {
            FMsgRing = new MsgRing(Capacity);
            SLReport = new TStringList();
            FDiffLog = new TStringList();
        } 

        public virtual void DoOnUpdateUI(int UUIA, string m)
        {
        }
        ...
        {
            FNextBridgeID++;
            DoOnUpdateUI(UUIAction.SwitchID, Utils.IntToStr(FNextBridgeID));
        ...
        public int SendBackupAndLog(int SwitchID, string Backup, string Log)
        {
            ...
            DoOnUpdateUI(UUIAction.BackupID, Utils.IntToStr(FBackupID));
            DoOnUpdateUI(UUIAction.BackupMsgID, Utils.IntToStr(FBackupMsgID));
            DoOnUpdateUI(UUIAction.BackupSwitchID, Utils.IntToStr(FBackupSwitchID));

            //todo: check Delphi vs C#
            return FMsgRing.MsgID; //C#
            //result := FBackupMsgID; //Delphi
        }
        ...
        public void SendMsg(int SwitchID, string msg)
        {
            ...
            DoOnUpdateUI(UUIAction.MsgID, Utils.IntToStr(FMsgRing.MsgID));
        }
        ...
        public string GetBackup()
        {
            ...
            //return FBackup + crlf + FLog; //C#
            return FBackup + crlf + FLog + crlf + GetNewMessages(0,0); //Delphi
        }
        ...
        /// </summary>
        public int MsgID => FMsgRing.MsgID;
...
//        }
        ...
        /// <summary>
        /// includes the content of the integer fields, only for debugging
        /// </summary>
        ...
        {
            sb.Append("-- TLocalBridge.StatusReport");
            sb.Append("Backup.Length = " + Utils.IntToStr(FBackup.Length));
            sb.Append("Log.Length = " + Utils.IntToStr(FLog.Length));
            sb.Append("DiffLog.Count = " + Utils.IntToStr(FDiffLog.Count));
            sb.Append("- IDReport");
            IDReport(sb);
        }

        private void IDReport(StringBuilder sb)
        {
        ...
        {
            return false; //see TServerBridge
        }
